...
var (
	...
	source = types.DataSource{
		ID:   vulnerability.ELxr,
		Name: "eLxr Security Tracker",
...
type VulnSrc struct {
	...
	// Hold a map of codenames and major versions from debian distributions.json
	// e.g. "aria" => "12"
	...
	// Hold eLxr advisories
	// e.g. {"aria", "connman", "CVE-2021-33833"} => {"FixedVersion": 1.36-2.1~deb10u2, ...}
	bktAdvisories map[bucket]Advisory

	// Hold not-affected versions
	// e.g. {"aria", "linux", "CVE-2021-3739"} => {}
...
func (vs VulnSrc) parse(dir string) error {
	rootDir := filepath.Join(dir, elxrDir, "tracker")
	...
	// Parse ESA/*.json
	if err := vs.parseESA(rootDir); err != nil {
		return eb.Wrapf(err, "ESA error")
	}
...
func (vs VulnSrc) parseBug(dir string, fn func(bug) error) error {
	...
	err := utils.FileWalk(dir, func(r io.Reader, path string) error {
		...
		if err := fn(bg); err != nil {
			return eb.Wrapf(err, "parse eLxr bug error")
		}
		return nil
	})
	...
	err := vs.parseBug(filepath.Join(dir, cveDir), func(bug bug) error {
		...
		for _, ann := range bug.Annotations {
			...
			case "trixie":
				elxrRelease = "bianca"
			default:
				elxrRelease = ann.Release
			}

			bkt := bucket{
				codeName: elxrRelease, // It will be empty in the case of sid.
				pkgName:  ann.Package,
				vulnID:   cveID,
			}
			...
			kind := ann.Kind
			if latestVersion, ok := vs.pkgVersions[bucket{codeName: elxrRelease, pkgName: ann.Package}]; ok {
...
func (vs VulnSrc) parseESA(dir string) error {
	vs.logger.Info("Parsing ESA JSON files...")
	if err := vs.parseAdvisory(filepath.Join(dir, esaDir)); err != nil {
		return oops.Wrapf(err, "ESA parse error")
	}
	return nil
}
	...
	return vs.parseBug(dir, func(bug bug) error {
		...
		for _, ann := range bug.Annotations {
			...
			for _, vulnID := range vulnIDs {
				bkt := bucket{
					codeName: elxrRelease,
					pkgName:  ann.Package,
					vulnID:   vulnID,
				}
				eb := oops.With("vuln_id", vulnID).With("package_name", ann.Package).With("code_name", elxrRelease)
...
func (vs VulnSrc) commit(tx *bolt.Tx) error {
	...
	for sidBkt, sidVer := range vs.sidFixedVersions {
		...
		// Iterate all eLxr codenames
		elxrCodenames := []string{"aria", "bianca"}
		for _, code := range elxrCodenames {
			bkt := bucket{
				codeName: code,
				pkgName:  pkgName,
				vulnID:   cveID,
...
func (vs VulnSrc) Get(params db.GetParams) ([]types.Advisory, error) {
	eb := oops.In("eLxr").With("release", params.Release).With("package_name", params.PkgName)
